...
public class Enemy1 : BlinkableCharacter<EnemyStates>, IListenToGameplayEvents
{
    ...
    private void Deregister(){
        ...
        AudioSystem.Instance.PlayEffect("EnemyDead", 1);
    ...
    public void Pump(){
        ...
        SetAnimationFrame(_pumpingStacks, EnemyStates.Pumping);
        _pumpingStacks += 1;
        AudioSystem.Instance.PlayEffect("Pump", 1);
    ...
    protected override EnemyStates UpdateTransitions(){
        ...
        switch (ActiveState) {
            ...
            case EnemyStates.Move:
                if(IsDeadByBlinking(TIMES_TO_DEAD_BY_TILE)) return EnemyStates.Dead;
                    ...
                    _elapsedStareTimeColdown <= 0){
                    _elapsedStareTime = TIME_OF_STARING;
                    AudioSystem.Instance.PlayEffect("EnemyStare", 1, true);
                    return EnemyStates.Stare;
                }
                break;
            case EnemyStates.Stare:
                if(IsDeadByBlinking(TIMES_TO_DEAD_BY_TILE)) return EnemyStates.Dead;
                else if(_pumpingStacks != 0) return EnemyStates.Pumping;
                else if(Vector3.Distance(transform.position, DigDugger.Player.transform.position) > 4f) {
                    _elapsedStareTimeColdown = TIME_OF_STARING_COLDOWN;
                    return EnemyStates.Move;
                } // LookingForPlayer
                else if(_elapsedStareTime <= 0){
                    _elapsedStareTimeColdown = TIME_OF_STARING_COLDOWN;
                    return EnemyStates.Move;
                } 
                break;
