...
public static class HighScoreRanking{
    ...
    public const int NUMBER_OF_RECORDS = 8;
...
//            Debug.Log(_ranking[i].Key + " " + _ranking[i].Value);
        ...
        CUtils.PrintContainer(_ranking);
    }

    public static bool IsTimed(GameType _type){ return _isTimed[_type];}
    ...
    public static List<KeyValuePair<int, string>> GetRankingCopy(){
        List<KeyValuePair<int,string>> copy = new List<KeyValuePair<int,string>>();
        for(int i = 0; i < _ranking.Count; i++){
            copy.Add(new KeyValuePair<int,string>(_ranking[i].Key, _ranking[i].Value));
        }
        return copy;
    }
    ...
    public static KeyValuePair<int, string> GetScore(int index){
        if(index < NUMBER_OF_RECORDS){ return _ranking[index]; }
